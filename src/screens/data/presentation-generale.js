...
export default function PresentationGenerale({navigation}) {
...
const handleRefresh = () => {
  ...
  getPresentationGenerale(); // Appeler la fonction de récupération des données
...
useEffect(()=>{
  navigation.setOptions({ title: "Présentation générale" });
  getPresentationGenerale();
},[])


const getPresentationGenerale = () =>{
  setIsLoading(true);
  fetch(`https://adores.tech/api/data/presentation-generale.php`,{
    method:'post',
    header:{
...
// Erreur et Chargement --fin--
  ...
  style={[styles.btn, { opacity: (data && data.length > 0) ? 1 : 0.5 }]}
>
  <Text style={styles.btnText}>Accéder au menu visiteur</Text>
</TouchableOpacity>
