...
import {SafeAreaView,StyleSheet,View,FlatList,Image,Text,TouchableOpacity,ActivityIndicator,TextInput,Share} from 'react-native';
import FeatherIcon from 'react-native-vector-icons/Feather';
import { MaterialCommunityIcons } from '@expo/vector-icons';
import { GlobalContext } from '../../global/GlobalState';


export default function Partage({navigation}) {
...
const [refreshing, setRefreshing] = useState(false);
  const handleRefresh = () => {
    ...
    getPartage (); // Appeler la fonction de récupération des données
...
useEffect(()=>{
  getPartage();
  navigation.setOptions({ title: 'Partager' });
},[])


// liste
const getPartage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/liste-partage.php`, {
    headers: {
...
<View style={[styles.cardIcon, { backgroundColor: "#007BFF" }]}>
                <MaterialCommunityIcons color="white" name='share-variant' size={20} />
              </View>

     
        <View style={styles.userInfo}>
          <Text style={styles.userName}>{item.titre}</Text>
          <Text style={styles.userCode}>{item.details}</Text>
        </View>
