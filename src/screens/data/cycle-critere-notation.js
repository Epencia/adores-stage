...
export default function CycleCritereNotation({navigation}) {
...
// api recherche
  ...
  const handleRefresh = () => {
    ...
    getListeCycle(); // Appeler la fonction de récupération des données
    setRefreshing(false); // Indiquer que le rafraîchissement est terminé
  };


useEffect(()=>{
     navigation.setOptions({title: 'Cycles'});
     getListeCycle();
},[])


  // liste
  const getListeCycle  = async () => {
    setIsLoading(true);
   try {
...
{data.length > 0 ? (
  ...
  value={searchTerm}
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.userContainer} onPress={() => navigation.navigate('Liste des criteres de notation',{item})}>
...
/>
        ...
        <View style={styles.userInfo}>
          <Text style={styles.userName}>{item.intitule_cycle}</Text>
          <Text style={styles.userCode}>{item.code_cycle}</Text>
