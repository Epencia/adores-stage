...
export default function ListePlateforme({navigation,item}) {
  ...
  useEffect(()=>{
    navigation.setOptions({ title: "Agences d'emploi" });
  ...
  const getListePlateforme = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-plateforme.php`, {
      headers: {
  ...
  const searchItems = useMemo(() => {
    return () => {
      const filteredData = data.filter(item =>
        item.titre_plateforme.toLowerCase().includes(searchTerm.toLowerCase())||
