...
export default function MatiereLicence({navigation,item}){
  ...
  useEffect(()=>{
    navigation.setOptions({ title: 'Matières' });
    getContact();
  ...
  const getMatiere = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-matiere-licence.php?matricule=${user[0].matricule}`, {
      headers: {
    ...
    value={searchTerm}
     />
         ...
         <View style={{ flex: 5, justifyContent: 'center', alignItems: 'center',backgroundColor:'white' }}>
           <MaterialCommunityIcons color="#266EF1" name="shield-off" size={150}/>
           <Text style={{ fontSize: 18,marginRight:10,marginLeft:10,marginBottom:10,textAlign:'center'}}>
           Vous ne pouvez pas accéder à ce contenu tant que vous n'êtes pas inscrit en tant qu'étudiant.
           </Text>
           <TouchableOpacity onPress={() => { Linking.openURL(`https://wa.me/${contact}`); }} style={{ backgroundColor: "#1F41BB",paddingVertical: 10,paddingHorizontal: 20,borderRadius: 5,}}>
             <Text style={{ color: 'white',fontSize: 16,fontWeight: 'bold',textAlign: 'center', }}>S'inscrire</Text>
           </TouchableOpacity>
         </View>
        )}
      ...
      renderItem={({item}) => (
            <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('Cours par matiere',{item})}>
