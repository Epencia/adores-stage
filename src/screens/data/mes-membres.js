...
export default function MesMembres({navigation,item}) {
...
const handleRefresh = () => {
  ...
  getMesStagiaires(); // Appeler la fonction de récupération des données
...
useEffect(()=>{
navigation.setOptions({title: 'Membres'});
...
const intervalId = setInterval(getMesStagiaires2, delay);
...
const getMesStagiaires = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/liste-membre.php?matricule=${user[0].matricule}`, {
    headers: {
