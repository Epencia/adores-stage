...
export default function EcoleCabinet({navigation,item}) {
  ...
  useEffect(()=>{
    navigation.setOptions({ title: "Écoles & cabinets" });
  ...
  const getListePlateforme = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/ecole-cabinet.php`, {
      headers: {
  ...
  const getListePlateforme2 = async () => {
   try {
    const response = await fetch(`https://adores.tech/api/mobile/ecole-cabinet.php`, {
      headers: {
        'Cache-Control': 'no-cache',
      },
    });
  ...
  const searchItems = useMemo(() => {
    return () => {
      const filteredData = data.filter(item =>
        ...
        item.sigle_etablissement.toLowerCase().includes(searchTerm.toLowerCase())
      );
      return filteredData;
    };
  }, [data, searchTerm]);
 ...
 />
    ...
    <View style={{marginTop: 25, marginRight:15,marginLeft:15,
      ...
      <FlatList
        ...
        keyExtractor={(item) => item.code_etablissement}
        renderItem={({item}) => (
...
/>
      ...
      <View style={styles.videoInfo}>
        <Text style={styles.videoTitle}>{item.nom_etablissement}</Text>
        <Text style={styles.videoChannel}>{item.sigle_etablissement}</Text>
      </View>
    </View>
