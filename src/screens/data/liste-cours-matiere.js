...
export default function CoursParMatiere({navigation,route}){
...
const {item} = route.params;
  ...
  const getMatiere = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-cours-matiere.php?classe=${item.classes}&matiere=${item.code_matiere}`, {
      headers: {
   ...
   try {
    ...
    setError(error);
      ...
      const filteredData = data.filter(item =>
        item.titre_cours.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.ordre_cours.toLowerCase().includes(searchTerm.toLowerCase())
      );
      return filteredData;
    };
  }, [data, searchTerm]);
  ...
  return (
    ...
    value={searchTerm}
     />
      ...
      renderItem={({item}) => (
            <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('PDF URL',{item})}>
              ...
              <View style={styles.cardBody}>
                <Text style={styles.cardTitle}>{item.titre_cours}</Text>
                <Text style={styles.cardCategory}>Cours {item.ordre_cours}</Text>
              </View>
