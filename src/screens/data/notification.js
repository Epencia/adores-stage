...
export default function ListeNotification ({navigation,item}){
...
const getListeMessage  = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/notification.php?id=${user[0].id}`, {
    headers: {
...
// In des mobile money
    ...
    const SuppressionNotification = (arg1,arg2) =>{
      fetch(`https://adores.tech/api/data/suppression-notification.php?id=${arg1}&user=${arg2}`,{
        method:'post',
          header:{
      ...
      const ValidationNotification = (notification) =>{
        ...
        fetch(`https://adores.tech/api/data/validation-notification.php?id=${notification.id}&user=${user[0].id}`,{
          method:'post',
            header:{
                'Accept': 'application/json',
                'Content-type': 'application/json'
