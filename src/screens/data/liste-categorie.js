...
const Categorie = ({item,navigation}) => {
...
const fetchData = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/categorie.php`, {
    headers: {
    ...
    <View style={{marginTop: 25, marginRight:15,marginLeft:15,
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          ...
          onPress={() => navigation.navigate("Partenaire par categorie",{item})}>
...
/>
            ...
            <Text style={styles.DoctorCategorie}>{item.titre}</Text>
          </TouchableOpacity>
        )}
