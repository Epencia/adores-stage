...
export default function AttestationStage({navigation,item}) {
  ...
  const handleRefresh = () => {
    ...
    getAttestationStage(); // Appeler la fonction de récupération des données
...
const intervalId = setInterval(getAttestationStage2, delay);
// Nettoyer l'intervalle lorsque le composant est démonté ou lorsque l'effet se réexécute
return () => clearInterval(intervalId);
},[])

// liste
const getAttestationStage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/liste-attestation.php?matricule=${user[0].matricule}`, {
    headers: {
...
// fin liste
const getAttestationStage2 = async () => {
 try {
  const response = await fetch(`https://adores.tech/api/data/liste-attestation.php?matricule=${user[0].matricule}`, {
    headers: {
      'Cache-Control': 'no-cache',
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.userContainer} onPress={() => navigation.navigate('Details attestation de stage',{item})}>
...
/>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.userCode}>* {item.type_stage}</Text>
        </View>
