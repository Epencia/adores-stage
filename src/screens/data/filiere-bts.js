...
export default function FiliereBTS({navigation,item}) {
...
useEffect(()=>{
    navigation.setOptions({title: "Filières"});
    getFiliereParCycle();
},[])
...
const getFiliereParCycle = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/liste-filiere-bts.php`, {
    headers: {
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.userContainer} onPress={() => navigation.navigate('Matiere par filiere',{item})}>
