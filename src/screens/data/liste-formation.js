...
return () => clearInterval(intervalId);
  ...
  const getFormations = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-formation.php?categorie=${item.code}`, {
      headers: {
...
/>
      ...
      {expandedItems.includes(item.id_formation) && (
          ...
          <View>
            ...
            <TouchableOpacity
              style={styles.followButton2} onPress={() => navigation.navigate('Videos par formation',{item})}>
