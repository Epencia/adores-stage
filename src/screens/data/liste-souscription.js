...
export default function ListeSouscription({navigation,item}) {
    ...
    const handleRefresh = () => {
      ...
      getListeSouscription(); // Appeler la fonction de récupération des données
  ...
  useEffect(()=>{
  navigation.setOptions({ title: 'Certificats' }); 
  // Exécuter la fonction avec cache
  const delay = 10000; // Définir le délai à 1 minute
  getListeSouscription(); 
  // Définir un intervalle pour exécuter la fonction sans cache toutes les 1 minute
  const intervalId = setInterval(getListeSouscription2, delay);
  // Nettoyer l'intervalle lorsque le composant est démonté ou lorsque l'effet se réexécute
  return () => clearInterval(intervalId);
  },[])
  
// Liste
  const getListeSouscription = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-souscription.php?matricule=${user[0].matricule}`, {
      headers: {
  ...
  // In des mobile money
  const getListeSouscription2 = async () => {
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-souscription.php?matricule=${user[0].matricule}`, {
      headers: {
        'Cache-Control': 'no-cache',
  ...
  const searchItems = useMemo(() => {
    return () => {
      const filteredData = data.filter(item =>
        ...
        item.prix_souscription.toLowerCase().includes(searchTerm.toLowerCase())||
 ...
 />
    ...
    <View style={{marginTop: 25, marginRight:15,marginLeft:15,
      ...
      <FlatList
        ...
        <TouchableOpacity style={styles.videoContainer} onPress={() => navigation.navigate('Videos par formation',{item})}>
                       {item.photo64 ? (
            <Image
alt=""
resizeMode="cover"
...
/>
      ...
      <View style={styles.videoInfo}>
        <Text style={styles.videoTitle}>{item.titre_formation}</Text>
        <Text style={styles.videoChannel}>{item.diplome}</Text>
      </View>
    </TouchableOpacity>
