...
export default function ListeMessage ({navigation,item}){
...
const handleRefresh = () => {
 ...
 useEffect(()=>{
  navigation.setOptions({title: 'Boite de réception'});
...
const getListeMessage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/data/liste-message.php?matricule=${user[0].matricule}`, {
    headers: {
...
const searchItems = useMemo(() => {
  ...
  const filteredData = data.filter(item =>
    ...
    item.contenu_message.toLowerCase().includes(searchTerm.toLowerCase()) ||
 ...
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        ...
        <TouchableOpacity  onPress={() => toggleItem(item.id_message)}>
...
/>
        ...
        {expandedItems.includes(item.id_message) && (
          <View>
        <Text style={styles.description}>{item.contenu_message}</Text>
        <View style={{marginBottom:20}}></View>
    <TouchableOpacity
      style={styles.followButton} onPress={() => navigation.navigate('Messages',{item})}>
      <Text style={styles.followButtonText}>Répondre</Text>
    </TouchableOpacity>
        </View>
      )}
