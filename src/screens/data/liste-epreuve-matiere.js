...
export default function EpreuveParMatiere({navigation,route}){
...
const {item} = route.params;
  ...
  useEffect(()=>{
    navigation.setOptions({ title: item.libelle_matiere });
  ...
  const getMatiere = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/liste-epreuve-matiere.php?filiere=${item.filiere}&matiere=${item.code_matiere}`, {
      headers: {
   ...
   try {
    ...
    setError(error);
      ...
      const filteredData = data.filter(item =>
        item.titre_epreuve.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.categorie_epreuve.toLowerCase().includes(searchTerm.toLowerCase())
      );
      return filteredData;
    };
  }, [data, searchTerm]);
  ...
  return (
    ...
    value={searchTerm}
     />
      ...
      keyExtractor={item=>item.code_epreuve} 
      renderItem={({item}) => (
            <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('PDF',{item})}>
              ...
              <View style={styles.cardBody}>
                <Text style={styles.cardTitle}>{item.titre_epreuve}</Text>
                <Text style={styles.cardCategory}>{item.categorie_epreuve}</Text>
              </View>
