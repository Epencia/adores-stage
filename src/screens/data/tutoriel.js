...
export default function Tutoriels({navigation,item}) {
...
useEffect(()=>{
 ...
 return () => clearInterval(intervalId);
  ...
  const getListeVideos = async () => {
    setIsLoading(true);
   try {
    const response = await fetch(`https://adores.tech/api/data/tutoriel.php`, {
      headers: {
  ...
  const getListeVideos2 = async () => {
   try {
    const response = await fetch(`https://adores.tech/api/mobile/tutoriel.php`, {
      headers: {
        'Cache-Control': 'no-cache',
      },
    });
