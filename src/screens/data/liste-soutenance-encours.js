...
export default function SoutenanceEncours({navigation,item}) {
...
useEffect(()=>{
navigation.setOptions({title: 'Soutenances autorisées'});
...
const getListeInscritsStage = async () => {
  setIsLoading(true);
 try {
  const response = await fetch(`https://adores.tech/api/masters/liste-soutenance-encours.php`, {
    headers: {
...
value={searchTerm}
 />
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <TouchableOpacity style={styles.userContainer} onPress={() => navigation.navigate('Details stagiaires',{item})}>
